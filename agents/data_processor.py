...
class DataProcessorAgent(BaseAgent):
    ...
    # Words to remove from product names for comparison
    NOISE_WORDS = [
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        Args:
            state: Current workflow state with raw product data

        Returns:
            AgentState: State with processed and deduplicated products
        """
        ...
        try:
            ...
            # Step 4: Sort by score (rating * log(reviews) + price factor)
    ...
    def _clean_product(self, product: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Cleaned product or None if invalid
        """
        try:
            # Required field: name
    ...
    def _normalize_rating(self, rating: Any) -> Optional[float]:
        ...
        try:
            ...
            if value > 5:
                value = value / 2  # Handle 10-point scale
    ...
    def _deduplicate_products(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Groups similar products and keeps the best one from each group.

        Args:
            products: List of cleaned products
    ...
    def _are_similar(self, p1: Dict, p2: Dict) -> bool:
        ...
        Args:
            p1, p2: Product dictionaries

        Returns:
            True if products are similar
        """
        # Quick check: same platform = definitely different listings
        # (we want to merge across platforms, not within)
        if p1.get("platform") == p2.get("platform"):
            # Same platform - only merge if exact match
        ...
        # Also check if brands match (if available)
        brand_match = True
    ...
    def _select_best_product(self, group: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        Criteria: best rating, most reviews, lowest price.

        Args:
            group: List of similar products

        Returns:
            Best product from the group
        """
        if len(group) == 1:
            return group[0]
        ...
        def score_product(p: Dict) -> Tuple[float, int, float]:
            ...
            # Score: higher rating better, more reviews better, lower price better
    ...
    def _sort_by_relevance(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        - Discount (bonus)
        - Availability (penalty for out of stock)
        ...
        def calculate_score(p: Dict) -> float:
            ...
            # Review count bonus (logarithmic to prevent domination)
